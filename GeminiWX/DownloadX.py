...
class TwitterVideoDownloader:
    ...
    def download_video(self, tweet_url: str, videoid:int, output_dir: Path = None):
        """
    ...
    def _download_video_variants(
        ...
        """
        if not variants:
            logger.error("No video variants available.")
            return

        # Select the highest bitrate variant
        highest_variant = max(variants, key=lambda v: v.bitrate)

        output_dir = output_dir or Path.cwd()
        output_dir.mkdir(parents=True, exist_ok=True)

        output_path = output_dir / f"Videos/{tweet_id}.mp4"
        self._download_file(highest_variant.url, output_path)
        logger.info(f"Downloaded highest resolution video: {output_path}")
...
# def downloadx(tweet_url_link):
#     tweet_url = tweet_url_link
...
#     downloadx("https://x.com/ValaAfshar/status/1902372699445276935")